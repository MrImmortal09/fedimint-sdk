...
mod activity;
// Android-only, and not part of the SDK surface: it exports `JNI_OnLoad` so the
// platform handles this crate's dependencies expect (`ndk_context`'s `JavaVM` and
// `Context`) are in place before anything reads them. Nothing calls into it.
