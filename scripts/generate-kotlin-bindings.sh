...
# `JNI_OnLoad` — only ART's own loader does — and `JNI_OnLoad` is where
# rust/fedimint-sdk/src/android.rs hands the platform's JavaVM and Context to
# the DNS resolver underneath iroh. Without it, the first dial to an
...
// library directly, and a bare `dlopen` never runs the library's `JNI_OnLoad` — which is where
// the Rust side hands the platform's JavaVM and Context to the DNS resolver underneath iroh.
