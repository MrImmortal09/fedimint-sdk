...
class MainActivity : AppCompatActivity() {
    ...
    private fun onRecover() {
        ...
        section(R.id.joinResult) {
            ...
            withContext(Dispatchers.Main) {
                ...
                watch(result, header) { recovery.progress.updates().next() }
            }
    ...
    private fun onOnchainSend() {
        ...
        section(R.id.onchainSendResult) {
            ...
            withContext(Dispatchers.Main) { watch(result, header) { operation.updates().next() } }
     ...
     */
    private fun watch(view: TextView, header: String, next: suspend () -> Any?) {
        lifecycleScope.launch {
            try {
                while (true) {
                    val state = withContext(Dispatchers.IO) { next() } ?: break
                    view.text = "$header\n\nstate: ${describeState(state)}"
                }
            } catch (e: SdkException) {
                view.text = "$header\n\n${describe(e)}"
